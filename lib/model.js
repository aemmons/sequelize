 ...
 * @param  {Array}        [options.insert_fields=Object.keys(this.attributes)] The fields to insert / update. Defaults to all fields
 * @param  {Array}        [options.update_fields=Object.keys(this.attributes)] The fields to insert / update. Defaults to all fields
 * @param  {Transaction}  [options.transaction] Transaction to run query under
 * @param  {Function}     [options.logging=false] A function that gets executed while running the query to log the sql.
 * @param  {Boolean}      [options.benchmark=false] Pass query execution time in milliseconds as second argument to logging function (options.logging).
 * @param  {String}       [options.searchPath=DEFAULT] An optional parameter to specify the schema search_path (Postgres only)
...
Model.prototype.upsert = function (values, options) {
  ...
  if (!options.insert_fields) {
    options.insert_fields = Object.keys(this.attributes);
  }

  if (!options.update_fields) {
    options.update_fields = Object.keys(this.attributes);
  }
  ...
  return instance.hookValidate(options).bind(this).then(function () {
    ...
    var updatedDataValues = _.pick(instance.dataValues, Object.keys(instance._changed))
      , insertValues = Utils.mapValueFieldNames(instance.dataValues, options.insert_fields, this)
      , updateValues = Utils.mapValueFieldNames(updatedDataValues, options.update_fields, this)
